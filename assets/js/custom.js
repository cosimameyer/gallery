...
/* ---------------------------------------------------------------
   A–Z nav: highlight the active letter pill.
   - Click sets it immediately.
   - IntersectionObserver keeps it in sync while scrolling.
--------------------------------------------------------------- */
(function setupAzNavHighlight() {
  const nav = document.querySelector("nav.az-nav");
  if (!nav) return;
  ...
  function setActive(letter) {
    links.forEach((l) => l.classList.toggle("active", l.textContent.trim() === letter));
  }

  // Click: mark immediately
  links.forEach((link) => {
    link.addEventListener("click", () => setActive(link.textContent.trim()));
  });

  // Scroll: watch each portrait-X anchor
  const anchors = Array.from(document.querySelectorAll("[id^='portrait-']"));
  ...
  const observer = new IntersectionObserver(
    (entries) => {
      ...
      if (visible.length) {
        const letter = visible[0].target.id.replace("portrait-", "").toUpperCase();
        setActive(letter);
      }
    },
